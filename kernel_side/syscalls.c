...
void syscalls_setVariable(void *ustack) {
	GETFROMSTACK(ustack, int, global_var, 0);
}
...
int syscalls_getLongestPath(void *ustack) {
	path_info_t *user_info;
	int skip_pid;

	GETFROMSTACK(ustack, int, skip_pid, 0);
	GETFROMSTACK(ustack, path_info_t *, user_info, 1);

	process_t *curr = proc_current()->process;

	if (user_info != NULL && curr != NULL) {
		user_info->max_depth = 1; 
		user_info->target_pid = curr->idlinkage.id; 
	}
	return 0;
}
